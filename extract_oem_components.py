...
def extract_okhttp(device):
    print("Find the OkHttp")
    folder_path = device
    file_names = ['okhttp.jar','boot-okhttp.oat','okhttp.odex']
...
def extract_bouncy(device):
    folder_path = device
    file_names = ['bouncycastle.jar','boot-bouncycastle.oat','bouncycastle.odex']

    dest_parent = os.path.join(config.current_path,'decompilations/jce/bouncycastle')
